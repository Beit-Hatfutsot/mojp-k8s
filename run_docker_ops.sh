...
#
# File can be downloaded and executed directly, assuming you have secret-mojp-k8s-ops.json available in current directory
#
# wget https://raw.githubusercontent.com/Beit-Hatfutsot/mojp-k8s/master/run_docker_ops.sh && chmod +x run_docker_ops.sh
# ./run_docker_ops.sh "production" "kubectl get pods"
#
...
[ -z "${OPS_DOCKER_IMAGE}" ] && OPS_DOCKER_IMAGE="orihoch/mojp-k8s"
[ -z "${OPS_REPO_SLUG}" ] && OPS_REPO_SLUG="Beit-Hatfutsot/mojp-k8s"
[ -z "${OPS_REPO_BRANCH}" ] && OPS_REPO_BRANCH="master"
[ -z "${OPS_SECRET_JSON_FILE}" ] && OPS_SECRET_JSON_FILE="`pwd`/secret-mojp-k8s-ops.json"
